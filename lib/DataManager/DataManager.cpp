...
void DataManager::addressToInt(std::string address, uint8_t& i1, uint8_t& i2, uint8_t& i3, uint8_t& i4)
{
	...
	dot1pos = address.find('.');
	dot2pos = address.find('.', dot1pos);
	dot3pos = address.find('.', dot2pos);
...
void DataManager::setMqttPort(uint16_t mqttPort)
{
	this->EEPROMwrite2bNumber(DM_MQTT_ADDR_START + DM_MQTTSERVER_MEM, DM_MQTTPORT_MEM, mqttPort);
}
...
uint16_t DataManager::getMqttPort()
{
	return this->EEPROMreadNumber(DM_MQTT_ADDR_START + DM_MQTTSERVER_MEM, DM_MQTTPORT_MEM);
}
